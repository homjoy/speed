 ...
 * @date 2015-09-15
 */
class DepartmentSub extends BasePackage {
    ...
    public static function get($params) {
        $cacheKey = MEM_PREFIX.'Department:DepartSub:';
       ...
       if (!empty($cacheData)) {
        ...
        //查询
        $result = self::getClient()->call('atom', 'department/depart_sub_list', $params);
	...
	public static  function create($params = array()) {
		...
		$result = self::getClient()->call('atom', 'department/create_depart_sub', $params);
	...
	public static function update($params = array()) {
		...
		$result = self::getClient()->call('atom', 'department/update_depart_sub', $params);
		if($result['httpcode'] != 200 || empty($result['content'])) {
			return FALSE;
		}else {
