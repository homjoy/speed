 ...
 * @date 2015-08-26
 */
class DepartmentInfo extends BasePackage {
    ...
    public static function get($params) {
        $cacheKey = MEM_PREFIX.'Department:DepartInfo:';
        ...
        //查询
        $result = self::getClient()->call('atom', 'department/dept_info_list', $params);
	...
	public static  function create($params = array()) {
		if(!is_array($params) || empty($params['depart_name'])) {
			return FALSE;
		}
	
		$result = self::getClient()->call('atom', 'department/create_dept_info', $params);
	...
	public static function update($params = array()) {
		...
		$result = self::getClient()->call('atom', 'department/update_dept_info', $params);
		if($result['httpcode'] != 200 || empty($result['content'])) {
			return FALSE;
		}else {
