...
class Curl extends \Libs\Serviceclient\CurlBase {
    ...
    private $useragent = 'Speed Webapi Connect';
    ...
    public function setHeaders($headers) {
        foreach ($headers as $key => $value) {
            RemoteHeaderCreator::instance()->setHeaders($key, $value);
        }
    }

    private function getHeaders() {
        $headers = RemoteHeaderCreator::instance()->getHeaders();

        $headerArr = array();
        $allow = array('Clientip', 'Xref', 'Speed', 'Cookie');
        foreach ($headers as $key => $value) {
