<?php
namespace Libs\Sphinx;

Use \Libs\Sphinx\Curl; 
//
// $Id: sphinxapi.class.php,v 1.1 2010-07-20 12:13:32 yuyang Exp $
//
...
class SphinxClient
{
	...
	var $_note = '';	///< add by zx, sending something to Mr.Su;
	...
	//get the info of sphinx server
    function getServerInfo() 
    {
        return array('host' => $this->_host, 'port' => $this->_port);
    }
	...
	function SetFilter ( $attribute, $values, $exclude=false )
	{
		...
		if ( is_array($values) && count($values) )
		{
			...
			$this->_filters[] = array ( "type"=>SPH_FILTER_VALUES, "attribute"=>$attribute, "exclude"=>$exclude, "values"=>$values );
		}
	}
	...
	function SetFilterRange ( $attribute, $min, $max, $exclude=false )
	{
		...
		$this->_filters[] = array ( "type"=>SPH_FILTER_RANGE, "attribute"=>$attribute, "exclude"=>$exclude, "min"=>$min, "max"=>$max );
	}
	...
	function SetFilterFloatRange ( $attribute, $min, $max, $exclude=false )
	{
		...
		$this->_filters[] = array ( "type"=>SPH_FILTER_FLOATRANGE, "attribute"=>$attribute, "exclude"=>$exclude, "min"=>$min, "max"=>$max );
	}
    ...
    function Query ( $query, $index="*", $comment="" )
    {
        ...
        $filters = array();
        $filterfloatranges = array();
        $filterranges = array();
        foreach ( $this->_filters as $filter )
        {
            if($filter['type'] == SPH_FILTER_VALUES)
            {
                $filters[] = array("attribute"=>$filter['attribute'], "exclude"=>$filter['exclude'], "value"=>$filter['values']);
            }
            else if($filter['type'] == SPH_FILTER_FLOATRANGE)
            {
                $filterfloatranges[] = array("attribute"=>$filter['attribute'], "exclude"=>$filter['exclude'], "min"=>$filter['min'], "max"=>$filter['max']);	
            }
            else if($filter['type'] == SPH_FILTER_RANGE)
            {
                $filterranges[] = array("attribute"=>$filter['attribute'], "exclude"=>$filter['exclude'], "min"=>$filter['min'], "max"=>$filter['max']);
            }
        }

        $url = "http://$this->_host:$this->_port/search/query";
        $post_data = array('query' => array('query' => $query, 'index' => $index),
                'limits' => array('offset' => $this->_offset, 
                    'limit' => $this->_limit, 
                    'maxmatches' => $this->_maxmatches, 
                    'cutoff' => $this->_cutoff),
                'matchmode' => $this->_mode,
                'rankingmode' => $this->_ranker,
				'select' => $this->_select,
                'sortmode' => array('sortmode' => $this->_sort, 'sortclause' => $this->_sortby),
                'online' => $this->_is_online,
				'note' => $this->_note
                );
        if (count($filters) > 0)
            $post_data['filter'] = $filters;
        if (count($filterfloatranges) > 0)
            $post_data['filterfloatrange'] = $filterfloatranges;
        if (count($filterranges) > 0)
            $post_data['filterrange'] = $filterranges;
		if (count($this->_overrides) > 0)
			$post_data['override'] = $this->_overrides;

        $post_json = json_encode($post_data);
		//if (json_last_error()) {
		//	    $log = new \Phplib\Tools\Liblog("json_error_sphinx", "normal");
		//		$log->w_log(print_r($_SERVER, TRUE));
		//		$log->w_log(print_r($post_data, TRUE));
		//}
        $curlHandle =  new curl();
        $pageInfo = $curlHandle->post($url, $post_json);
        if ( false === $pageInfo ){
            return False;
        }
        ...
        if (array_key_exists('code',$results) && $results['code'] == -1)
            return false;

        if ( !$this->_arrayresult )
        {
            $tmp_match = $results[0]['matches'];
            $results[0]['matches'] = array();
            foreach ($tmp_match as $match)
            {
                $results[0]['matches'][$match['id']] = array('weight' => $match['weight'], 'attrs' => $match['attrs']);
            }
        }
	...
	function UpdateAttributes ( $index, $attrs, $values, $sync = 1)
	{
		$sync = (int)$sync;
		//这里检查数据的类型是否符合要求
        assert ( is_string($index) );
        ...
        include_once 'curl.class.php';
        $curlHandle =  new curl();
        $url = "http://$this->_host:$this->_port/search/update";
        $pageInfo = $curlHandle->post($url, $data);
		$code = -2;
		if (isset($pageInfo['body'])) {
			$body = $pageInfo['body'];
			$decodeBody = json_decode($body);
			$code = $decodeBody->code;
		}
		return $code;
	}
