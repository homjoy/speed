...
class curl
{
    ...
    var $cookie = false;
    var $proxy = "";    //add for proxy by liangbo 2012.8.2
    var $timeout = 10;
...
//			$err = curl_error($ch);
    ...
    function doRequest($method, $url, $vars, $referer = '', $is_webservice = false ,$is_urlencode= false )
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_FRESH_CONNECT, 1);
        curl_setopt($ch, CURLOPT_URL, $url);
        ...
        if ($is_webservice) {
            curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: text/xml; charset=utf-8", "Content-length: " . strlen($vars)));
        }
        if( $is_urlencode ){
    ...
    function post($url, $vars, $is_webservice = false,$is_urlencode=false)
    {
        return $this->doRequest('POST', $url, $vars, '', $is_webservice,$is_urlencode);
    }
}
