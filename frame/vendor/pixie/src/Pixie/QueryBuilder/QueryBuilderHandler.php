...
class QueryBuilderHandler
{
    ...
    public function __construct(Connection $connection = null)
    {
        ...
        $databaseType = $this->connection->getAdapter()->getDatabaseType();
        $this->adapterInstance = $this->container->build('\\Pixie\\QueryBuilder\\Adapters\\' . ucfirst($databaseType),
            array($this->connection,$this->container));
    }
    ...
    public function get()
    {
        ...
        $fetchParam = isset($this->statements['hashKey']) ? array('hashKey'=>$this->statements['hashKey']) : array();

        list($result,$executionTime) = $this->connection->read(
            $queryObject->getSql(),
            $queryObject->getBindings(),
            $this->fromMaster,$fetchParam);
    ...
    public function getQuery($type = 'select', $dataToBePassed = array())
    {
        ...
        return $this->container->build(
            ...
            array($queryArr['sql'], $queryArr['bindings'],$this->adapterInstance)
        );
    }
    ...
    private function doInsert($data, $type)
    {
        ...
        if (!is_array(current($data))) {
            ...
            list($result,$executionTime) = $this->connection->write($queryObject->getSql(), $queryObject->getBindings());

            $return = $result === 1 ? $this->connection->lastInsertId() : null;
        } else {
            // Its a batch insert
            $return = array();
            $executionTime = 0;
    ...
    public function update($data)
    {
        ...
        list($response,$executionTime) = $this->connection->write($queryObject->getSql(), $queryObject->getBindings());
    ...
    /**
     * is read/write from master db connection
     * @param $master
     * @return $this
     */
    public function fromMaster($master)
    {
        $this->fromMaster = $master ? true : false;
        return $this;
    }

    /**
     * convert result to hash map by given key.
     * @param $key
     * @return $this
     */
