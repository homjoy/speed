...
abstract class BaseAdapter
{
    ...
    /**
     * bind params style , allow['?','key']
     * @var string
     */
    protected $bindStyle = 'key';
    ...
    private function getUpdateStatement($data)
    {
        ...
        foreach ($data as $key => $value) {
            if ($value instanceof Raw) {
                ...
                switch($this->bindStyle){
                    case 'key':
                        $statement .= $this->wrapSanitizer($key) . '= :'.$key.',';
                        $bindings[$key] = $value;
                        break;
                    case '?':
                    default:
    ...
    protected function buildCriteria($statements, $bindValues = true)
    {
        ...
        foreach ($statements as $statement) {
            ...
            if($key instanceof \Closure){
                ...
                $normalKey = str_replace('.','_',$statement['key']);
            }
            ...
            if (is_null($value) && $key instanceof \Closure) {
                ...
                switch ($statement['operator']) {
                    case 'BETWEEN':
                        switch($this->bindStyle){
                            case 'key':
                                if(count($statement['value']) !== 2){
                                    throw new Exception('BETWEEN must provide two value!');
                                }
                                $betweenStart = $this->getValueKey($normalKey);
                                $betweenEnd = $this->getValueKey($normalKey);
                                $bindings[$betweenStart] = $statement['value'][0];
                                $bindings[$betweenEnd] = $statement['value'][1];
                                $criteria .= ' :'.$betweenStart.' AND :'.$betweenEnd.' ';
                                break;
                            case '?':
                            default:
                    ...
                    default:
                        $valuePlaceholder = '';
                        foreach ($statement['value'] as $index=>$subValue) {
                            switch($this->bindStyle){
                                case 'key':
                                    $valueKey = $this->getValueKey($normalKey);
                                    $valuePlaceholder .= ':'.$valueKey.', ';
                                    $bindings[$valueKey] = $subValue;
                                    break;
                                case '?':
                                default:
                                    $valuePlaceholder .= '?, ';
                ...
                if (!$bindValues) {
                    ...
                    switch($this->bindStyle){
                        case 'key':
                            $normalKey = $this->getValueKey($normalKey);
                            $valuePlaceholder = ':'.$normalKey;
                            $bindings[$normalKey] = $value;
                            break;
                        case '?':
                        default:
                            $valuePlaceholder = '?';
    ...
    /**
     * quote value by sanitizer
     * @param $value
     * @return string
     */
    public function quote($value)
    {
        $value = str_replace($this->sanitizer, $this->sanitizer.$this->sanitizer,$value);
        return $this->wrapSanitizer($value);
    }
