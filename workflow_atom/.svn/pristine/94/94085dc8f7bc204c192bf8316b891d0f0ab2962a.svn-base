<?php
namespace WorkFlowAtom\Package\Treerole;
...
class Role  {
    ...
    private static $tableName = 'tree_role';
    ...
    public function getDataList(array $params = array())
    {
        ...
        if (isset($params['role_name'])) {
            if (is_array($params['role_name'])) {
                $ret->whereIn('role_name', $params['role_name']);
            }else{
                $ret->where('role_name', '=', $params['role_name']);
            }
        }
        $ret->orderBy('role_id', 'asc');
