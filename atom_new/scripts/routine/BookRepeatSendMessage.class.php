...
class BookRepeatSendMessage extends \Frame\Script {
    ...
    public function run() {
        ...
        $meetingRoom = MeetingRoom::getInstance()->getDataList($p,1,99);
        $meetingRoomId = ArrayUtilities::my_array_column($meetingRoom,'room_id');
        $date = date('Y-m-d H:i:s');
        $repeat_book = BookInfo::model()->getRepeatByDay($date,$meetingRoomId);
