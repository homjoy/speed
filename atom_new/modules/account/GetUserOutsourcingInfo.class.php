...
class GetUserOutsourcingInfo extends \Atom\Modules\Common\BaseModule {
    ...
    public function run() {
        ...
        if($this->params['out_user_id']!=''){
            $queryParams['out_user_id'] = $this->params['out_user_id'];
        }
        ...
        if(!empty($this->params['mail_suffix'])){
            $queryParams['mail_suffix'] = $this->params['mail_suffix'];
        }
        //校验是否有限定参数
        ...
        $result = UserOutsourcingInfo::model()->getDataList($queryParams,$this->params['offset'], $this->params['limit']);
