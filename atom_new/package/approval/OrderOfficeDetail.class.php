...
class OrderOfficeDetail extends BaseQuery{

    private static $col = array('id', 'order_id', 'supply_id', 'supply_name', 'detail_info', 'apply_number','update_time','status');
    ...
    public function getDataList($params = array(),$offset = 0,$limit = 100)
    {
        ...
        if (isset($params['supply_id'])) {
            if (is_array($params['supply_id'])) {
                $builder->whereIn('supply_id', $params['supply_id']);
            }else{
                $builder->where('supply_id', '=', $params['supply_id']);
            }
        }
