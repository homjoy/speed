...
class CreateUserInfo extends \Atom\Modules\Common\BaseModule {
    ...
    public function run() {
        ...
        if(isset($this->params['staff_id'])){
            ...
            $result = UserInfo::model()->getDataList(array('staff_id'=>$result,'match'=>'like'),0,100);
            if(!empty($result)){
        ...
        $result = UserInfo::model()->insert($this->params);
        if ($result === FALSE) {
