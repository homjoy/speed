 ...
 * @data 2015-11-13 下午5:37:16
 */
class VisitingCardSendMessage{
    ...
    public function sendMessage($params = array()){
        //名片单
        $order = OrderVisitingCard::model()->getById($params['order_id']);
        ...
        //admin
        $output_manager = Config::model()->getChild('/executive/card/output_manager');
        ...
        if(!empty($direct_leader)){
            foreach($direct_leader as $d_l){
                ...
                $content = array(
                    ...
                    'url'           => '/administration/businesscard/approval',
                );
                ...
                $mail_params = array(
                    ...
                    'template_id'   => 223,
                    'to_id'         => $d_l['user_id'],
                    'title'         => "【名片申请提醒】申请人：{$params['user_name']}",
                    'mobile'        => '',
                    'mail'          => $d_l['mail'],
                );
    ...
    private function _sendCompleteMessage($params){
        ...
        $content = array(
            'username'      => "{$params['user_name']}（{$params['depart_name']}）",
            'order_explains'=> '请安排印制吧！',
            'order_type'    => $params['order_type'],
            'order_id'      => $params['order_id'],
            'url'           => '/administration/businesscard/manage',
        );
        $mail_params = array(
            'token_type'    => 'process',
            'content'       => $content,
            'template_id'   => 219,
            'title'         => "【名片审批通过】申请人：{$params['user_name']}",
            'mobile'        => '',
        );
    ...
    private function _sendRejectMessage($params){
        ...
        $content = array(
            ...
            'order_type'    => $params['order_type'],
            'order_id'      => $params['order_id'],
            'url'           => '/administration/businesscard/my',
        );
        $mail_params = array(
            'token_type'    => 'process',
            'content'       => $content,
            ...
            'title'         => "【名片申请驳回】申请人：{$params['user_name']}",
            'mobile'        => '',
    ...
    private function _sendRevokeMessage($params){
        ...
        $mail_params = array(
            'token_type'    => 'process',
            'content'       => $content,
            'title'         => "【名片申请撤销】申请人：{$params['user_name']}",
            'mobile'        => '',
        );
    ...
    private function _sendGrantMessage($params){
        ...
        $content = array(
            ...
            'receive_place' => '26层前台',
            'order_type'    => $params['order_type'],
            'order_explains'=> "你的名片已准备好啦！领取地点：26层前台",
            'order_id'      => $params['order_id'],
        );
        $mail_params = array(
            'token_type'    => 'process',
