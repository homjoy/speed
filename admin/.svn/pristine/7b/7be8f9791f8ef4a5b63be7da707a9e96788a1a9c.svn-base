...
use Admin\Package\Itserver\MlsAccountList;
/**
 ...
 * Time: 下午12:18
 */
class AccountList extends BaseModule
{
    ...
    public function run()
    {
        ...
        $temp_count = MlsAccountList::getInstance()->getAccountList($count_params);
        //获取邮箱的列表
        $logs_list = array();
        if($temp_count >0){
            //获取数据
            ...
            if(!empty($logs_list)) {
                ...
                foreach ($logs_list as &$log_info) {
                    ...
                    $log_info['handle_type'] = isset($this->handle_type[$log_info['handle_type']]) ? $this->handle_type[$log_info['handle_type']]
                        : '默认';
                }
                unset($log_info);
            }
        ...
        if(!empty($this->params['user_id'])){
            $search_user_info =  isset($user_info_list[$this->params['user_id']]) ? $user_info_list[$this->params['user_id']]:
                array();
