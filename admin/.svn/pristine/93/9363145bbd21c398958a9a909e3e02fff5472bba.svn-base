        ...
        <div class="panel-body">
            ...
            <div id="page-content" class="panel">
                <ul class="nav nav-tabs slope col-lg-12 col-sm-12 col-xs-12" style="margin-bottom: 30px">
                    <li  class="item" role="presentation"><a href="/company/company_home">公司</a></li>
                    <li  class="active" role="presentation"><a href="/company/city_home" >城市</a></li>
                </ul>
            </div>
                <div class="from-container">
                    <form id="searchForm" class="form-inline  form-horizontal" role="form"
                          action="/company/city_home" method="get">
                        <div class="row">
                ...
                <table class="table table-striped table-hover" style="font-size:14px;">
                    ...
                    @if (!empty($data))
                        ...
                        @foreach ($data as $t)
                            <tr>
                                <td>{{ $t['city_id'] or ''}}</td>
                                <td>{{ $t['city_name'] or ''}}</td>
                                <td>{{ $t['status'] or '' }}</td>

                                <td>
                                    <a class="btn btn-sm btn-warning editBtn" type="button" data-toggle="modal"
                                       data-weather_id="{{$t['weather_id'] or ''}}"     data-pinyin="{{$t['pinyin'] or ''}}"  data-status="{{$t['status'] or ''}}"   data-name="{{$t['city_name']}}"  data-id="{{$t['city_id'] or ''}}"><span
                                         class="glyphicon glyphicon-log-in rm-disable"></span>修改
                                    </a>
                                    <a class="btn btn-sm btn-danger  deleteBtn" type="button" data-toggle="modal"
                                       data-name="{{$t['city_name'] or ''}}" data-id="{{$t['city_id'] or ''}}"><span class="glyphicon glyphicon-remove rm-disable"></span>删除
                                    </a>
                                </td>
...
<!--        </div>-->
                    ...
                    <div class="modal-body">
                        <form  id="addForm" class="form-horizontal">
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" readonly class="form-control m_id" name ='city_id'  placeholder="ID">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="depart_id" class="col-sm-2 control-label">名称</label>
                                <div class="col-sm-10">
                                    <input type="text"  class="form-control m_name" name ='city_name' placeholder="名称">
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        $(".pagination-left").pagination({
            ...
            location.href = "/company/city_home?page=" + page + '&status='+ $('#status').val();
        });
        ...
        $('#add_btn').click(function() {
            ...
            $.post('/company/ajax_update_add_city',myForm, function(ret) {
                if (ret.code == 200) {
                    show_message(ret.code,ret);
                    setTimeout("window.location.href = '/company/city_home'",1500);
                } else {
                    show_message(ret.code,ret);
                }
            }, 'json');
        ...
        $('.deleteBtn').click(function(){
            ...
            bootbox.confirm('确认删除吗？', function (result) {
                if (result) {
                    ...
                    $.ajax({
                        url: "/company/ajax_update_add_city",
                        method: "POST",
                        data: {city_id: _this.attr('data-id'),status:0,city_name:_this.attr('data-name')},
                        dataType: "json",
