...
//        'bank_card_number'   => '银行卡号',
	...
	public function run() {
        ...
        //获取部门领导相关信息
        $depart_info = Department::getInstance()->getDepart(array('depart_id' => implode(',', $departIds),'status'=>'1,0','all'=>1));
        ...
        if (is_array($depart_leader_info)) {
            $ret_temp = $ret =array();
            $ret = UserInfo::getInstance()->getUserInfo(array('all'=>1,'status'=>$this->params['status']));
            foreach ($depart_leader_info as $k => $v) {
                if(isset($v['user_id'])&& empty($v['user_id'])){
                    $temp  =  DepartSub::getInstance()->getSubInfo(array('relation_id'=>$v['relation_id'],'status'=>1));
                    $temp = is_array($temp)?array_pop($temp):'';
                    $depart_leader_info[$k]['name_cn'] =   isset($ret[$temp['user_id']]['name_cn'])?$ret[$temp['user_id']]['name_cn']:'';
                    continue;
                }
       ...
       if(is_array($user_info)){
            foreach($user_info as $k => $v){
                $list[$k] = $user_info[$k];
                $list[$k]['direct_leader'] = isset($user_info[$v['direct_leader']]['name_cn'])
                ?$user_info[$v['direct_leader']]['name_cn']:'';
                ...
                //性别
               if(isset($list[$k]['gender'])){
                   switch($list[$k]['gender'] ){
                       case 1:
                           $list[$k]['gender'] = '男';
                           break;
                       case  0:
    ...
    private function export($list= array()){
		...
		foreach ($list as $k => $v) {
			...
			while ($index < $count) {
                ...
                $objPHPExcel->getActiveSheet ()->setCellValueExplicit ($location,
                        self::getParamsByKey ($v, $index), 's');
				$index++;
			}
			$currRow++;
		}
    ...
    protected function doLog($new_param=array(),$old_param='export'){

        $ret = Log::getInstance()->createLogs(array(
                'user_id'=>$this->user['id'],//op
                'handle_id'=>100000,
