...
/**
 *  GetDepart 获取一个部门相关信息
 * hongzhou@meilishuo.com
 * 2015-09-25
 */
class AjaxGetDepart extends BaseModule {
    ...
    public function run() {
        ...
        $depart_info =  Department::getInstance()->getDepart($this->params);
        ...
        //parent
        if(isset($depart_info['parent_id'])){
            $parent = Department::getInstance()->getDepart(array('depart_id'=>
        ...
        $depart_info['parent_id'] = isset($parent['depart_id'])?$parent['depart_id']:'';
        $depart_info['parent_name'] =isset($parent['depart_name'])?$parent['depart_name']:'';
       //leader
        $leader_info =  $this->getRelationInfo(array('is_virtual' =>0,'depart_id'=>
        ...
        $ret['virtual_role_id'] = isset($leader_info['virtual_role_id'])?$leader_info['virtual_role_id']:'';
    ...
    private function getRelationInfo($params){//获取领导名字 信息
        $depart_leader_info = self::getClient()->call(
            'atom',
            'department/depart_relation_list',
            $params
        );
        ...
        if(is_array($depart_leader_info)){
            ...
            if(!empty($p_r)){
                $v_relation_info = self::getClient()->call(
                    'atom',
                    'department/depart_relation_list',
                    array(
                        'relation_id'=> $p_r,
                ...
                if(!empty($v_relation_info )){
                    ...
                    $depart_leader_info['virtual_role_id'] =isset($v_relation_info['role_id'])?$v_relation_info['role_id']:'';
                    $v_relation_info = isset($v_relation_info['user_id'])?$v_relation_info['user_id']:'';
                    $v_relation_info = self::getClient()->call(
                        'atom',
                        'account/get_user_info',
                        array(
                            'user_id'=>$v_relation_info,
                            'status'=>array(1,2,3)
                        )
                    );
                    $v_relation_info = $this->parseApiData($v_relation_info);
            ...
            //代理领导
            if(!empty($depart_leader_info['relation_id'])){
                $depart_leader_sub = self::getClient()->call(
                    'atom',
                    'department/depart_sub_temp_list',
                    array(
                        'relation_id'=>$depart_leader_info['relation_id'],
                    )
                );
                ...
                if($depart_leader_info['sub_leader_id'] !=''){
                    ...
                    $leader_sub_name = self::getClient()->call(
                        'atom',
                        'account/get_user_info',
                        array(
                            'user_id'=>$depart_leader_info['sub_leader_id'],
                            'status'=>array(1,2,3)
                        )
                    );
                ...
                $depart_leader_info['sub_name']  = '';
                $depart_leader_info['sub_leader_id'] ='';
            }
            //部门领导
            if(isset($depart_leader_info['user_id'])){
                $leader_name = self::getClient()->call(
                    'atom',
                    'account/get_user_info',
                    array(
                        'user_id'=>$depart_leader_info['user_id'],
                        'status'=>array(1,2,3)
                    )
                );
