...
(function ($) {
    $.fn.EasyTree = function (options) {
        ...
        //查看,修改 或其它按钮
        var speed_btn = '';
        if(options.speed_show_btn) {
            speed_btn += '<button type="submit" class="btn btn-default speed_show_model" data-toggle="modal">详情</button> ';
        }

        if(options.speed_edit_btn) {
            speed_btn += '<button type="submit" class="btn btn-default speed_add_model" data-toggle="modal">添加</button><button type="submit" class="btn btn-default speed_edit_model" data-toggle="modal">编辑</button>';
        }


        if(options.speed_delete_btn) {
            speed_btn += '<button type="submit" class="btn btn-default speed_delete_model" data-toggle="modal">删除</button>';
        }
        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    $(children).remove();
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span>&nbsp;<a href="javascript: void(0);"></a> &nbsp;&nbsp;' + speed_btn + '</span>');
                    ...
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span>&nbsp;<a href="javascript: void(0);"></a> &nbsp;&nbsp;' + speed_btn + '</span>');
                    $(this).find(' > span > span').addClass('glyphicon-user');
                    $(this).find(' > span > a').text(text);
                }
            });
