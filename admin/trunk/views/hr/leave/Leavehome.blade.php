    ...
    <div class="form-container">
            ...
            <div class="row form-group ">
                ...
                <div class="col-lg-2 col-xs-2 col-sm-2">
                    <select class="form-control" name="status" id="status" >
                        ...
                        <option value='1,2,3'>审批中</option>
                        <option value='4'>审批完成</option>
                        <option value='5,6'>审批驳回</option>
                    </select>
                </div>
                <div class="col-lg-2 col-xs-2 col-sm-2">
                    <select class="form-control" name="user_status" id="userStatus" >
                        <option value='' selected>全职位状态</option>
            ...
            <div class="row form-group">
                ...
                <div class="col-lg-4 col-xs-4 col-sm-4 button-groups">
                    ...
                    <button type="button" class="btn btn-primary" id="exportBtn" >流程导出</button>
                    <button type="button" class="btn btn-primary" id="exportLeaveBtn" >余假导出</button>
                </div>
            </div>
        </form>
    </div>
            ...
            <table class="table-hover table table-striped table-bordered">
                ...
                if(empty($data)){
                   ...
                   foreach($data as $key =>$v){
                       if(isset($v['status'])){
                           switch($v['status']){
                               ...
                               case 4:
                                   $strClass='success';
                                   $status ='审批完成';
                                   $editBtn = '<a type="button"  data-id="'.$v['order_id'].'" class="btn btn-sm btn-primary editBtn"><span class="glyphicon glyphicon-log-in rm-disable"></span>修改</a>';
                                   $cancleBtn ='<a type="button"  data-id="'.$v['order_id'].'" class="btn btn-sm btn-danger cancleBtn"><span class="glyphicon glyphicon-log-in rm-disable"></span>取消</a>';
                                break;
                               case 5:
                                   $strClass='danger';
                                   $status ='驳回';
                       ...
                       if(isset($v['memo'])){
                           $len = strlen($v['memo']);
                           if($len >=70){
                               $v['memo'] = mb_substr($v['memo'],0,70,'utf-8');
                               $v['memo'] .='...';
                           }
                ...
                <tr  order_id="{$v['order_id']}">
                    ...
                    <td>
                        <a data-task_id="{$v['task_id']}" data-date="从{$v['start_date']}{$v['start_half']}到 {$v['end_date']}{$v['end_half']}" data-memo="{$v['memo']}" id="{$v['order_id']}" class="btn btn-sm btn-default toggle-detail"><span class="glyphicon glyphicon-zoom-in"></span>查看</a>
                        {$editBtn}&nbsp;{$cancleBtn}
                    </td>
                </tr>
DOC;
...
<script>
    ...
    $('.toggle-detail').css('cursor','pointer').click(function(ev){
        ...
        str ='<tr><td colspan=4>请假申请</td></tr>'+'<tr >'+
            '<th >请假原因:</th><th colspan="3">'+
            $(this).attr('data-memo')+
            '</tr><th >请假时间:</th ><th colspan="3">'+
            $(this).attr('data-date')+
            '</tr>';
    ...
    $('#exportBtn').click(function() {
        show_message(200,'');
        window.location.href= '/hr/leave/export_leave_list?' +'end_time=' +$("#endTime").val()+ '&create_time='+$("#createTime").val()+'&depart_id='+$("#departId").val()+'&absence_type='+$('#absenceType').val()
            +'&user_id='+$('#userId').val()+'&user_status='+$('#userStatus').val()+'&status='+$('#status').val();

    });
    $('#exportLeaveBtn').click(function() {
        show_message(200,'');
        window.location.href= '/hr/leave/export?'+'depart_id='+$("#departId").val()
            +'&user_id='+$('#userId').val()+'&status='+$('#userStatus').val();
