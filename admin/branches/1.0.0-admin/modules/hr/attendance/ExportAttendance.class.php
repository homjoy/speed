...
class ExportAttendance extends BaseModule
{
    ...
    public function run()
    {
        ...
        if (!empty($work_staff_list)) {
            $work_user_info = ArrayUtilities::my_array_column($work_staff_list, 'depart_id');
            $depart_info_array = array_unique($work_user_info);
//            $params = array('user_id' =>implode(',', $work_user_info ), 'status' => array(1,  2,3), 'all' => 1);
//            $sign_user_info = UserInfo::getInstance()->getUserInfo($params);
//
//            if (!empty($sign_user_info)) {
//                $depart_info_array = ArrayUtilities::my_array_column($sign_user_info, 'depart_id');
//                $depart_info_array = array_unique($depart_info_array);
//            }
            if (!empty($depart_info_array)) {
                foreach ($depart_info_array as $depart_id) {
                    $depart_info = Department::getInstance()->getDepartInfo(array('depart_id' => $depart_id));
                    $current_depart_info = current($depart_info);
                    $attendance_depart_info[$current_depart_info['depart_id']]  = $current_depart_info;
                }
            }
            //给用户赋值 部门信息
//            foreach($sign_user_info as &$sign_user_info_value){
//                $sign_user_info_value['depart_name'] = isset($attendance_depart_info[$sign_user_info_value['depart_id']]) ?
//                    $attendance_depart_info[$sign_user_info_value['depart_id']]['depart_name'] : '';
//            }
//            unset($sign_user_info_value);
            foreach ($work_staff_list as $key => $work_staff_value) {
                ...
                //获取当前人的信息，抓取部门数据
                $depart_name =!empty(  $attendance_depart_info[ $work_staff_value['depart_id']]['depart_name'] ) ?  $attendance_depart_info[ $work_staff_value['depart_id']]['depart_name'] : '';
