...
//计算天数
        ...
        if(isset($this->params['calculate'])&&$this->params['calculate']==1){//只计算天数
            $return = Response::gen_success(array('length'=> $this->params['length']));
            return $this->app->response->setBody($return);
        }
//修改入库
 ...
 //记录logs
    ...
    protected function doLog($new_param=array(),$old_param='update'){
        ...
        $ret = Log::getInstance()->createLogs(array(
                ...
                'handle_id'=>isset($new_param['order_id'])?$new_param['order_id']:'',
