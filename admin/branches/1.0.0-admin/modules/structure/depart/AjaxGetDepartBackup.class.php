...
class AjaxGetDepartBackup extends BaseModule {
   ...
   public function run() {
       //部门信息 领导信息 部门下面人员信息
       $this->_init();
       if($this->query()->hasError()||$this->params['depart_id']===self::ROOT_DEPART){
       ...
       if(isset($depart_info['parent_id'])){
          ...
          $parent =is_array($parent)?array_pop($parent):$parent;      
      ...
      //leader  role sub leader vir
       $leader_info =  $this->getDepartLeaderInfo(
        array(
          'is_virtual' =>0,
       ...
       if($ret['parent_id']['parent_id']==self::ROOT_DEPART){
           $ret['parent_id']['parent_name'] ='美丽说';
       }else{
    ...
    private function getDepartLeaderInfo($params){//获取领导名字 信息
       $depart_leader_info = self::getClient()->call(
           'atom',
           'department/depart_relation_temp_list',
           $params
       );
