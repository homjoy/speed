...
//注册module的namespace
$app->scripts_namespace = '\\Joint\\Scripts\\';
...
$app->config->db = function() {
    return \Joint\Config\Web\MySQL::config();
};
$app->config->redis = function() {
    return \Joint\Config\Web\Redis::config();
};
$app->config->memcache = function() {
    return \Joint\Config\Web\Memcache::config();
};
$app->config->remote = function() {
    return \Joint\Config\Web\Remote::config();
};
