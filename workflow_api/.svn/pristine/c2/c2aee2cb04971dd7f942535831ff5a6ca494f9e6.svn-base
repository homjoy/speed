...
class TaskTransfer {
	...
	public static function operateTransfer ($params = array()) {
		...
		if(1 == $nextCount) {
			...
			if (!empty($ruleParams)) {
				$taskUpdateResult = self::taskUpdate(array(
						'task_id'	    => $taskInfo['task_id'],
						'task_content'	=> json_encode($ruleParams),
					), array()
				);
			}
		...
		if(empty($nextUserId)) {
			if(strstr($processInfo[$next_node_id]['role_id'],'speed_')) {
				...
				return TaskTransfer::operateTransfer(array(
					...
					'ruleExtendParams'  => !empty($ruleParams) ? $ruleParams : array(),
				));
			}else {
	...
	public static function getNextUser($role_id, $params = array()) {
		if(strstr($role_id, 'speed_')) {
			...
			}elseif(strstr($role_id, '_before')) {
				//逐级审批直到某职位之前
				$role_id = str_replace('_before', '', $role_id);
				return self::getNextUserByLeaderLine($role_id, $params, true, 'before');
			}if(strstr($role_id, '_after')) {
				//逐级审批直到某职位之后
				$role_id = str_replace('_after', '', $role_id);
				return self::getNextUserByLeaderLine($role_id, $params, true, 'after');
			}else {
				//非逐级，只是某一职位审批
	...
	private static function getNextUserByLeaderLine($sp_role, $params = array(), $step = true, $placeholder = '') {
		...
		foreach($leaderLineInfo as $key => $v) {
			
			if($step) {
				if (!empty($currentRoleId) && $v['role_id'] > $currentRoleId && $placeholder != 'placeholder') {
					continue;
				}

				if($v['role_id'] >= $sp_role && !in_array($v['leader_user_id'], $progressUid) && $params['operator'] != $v['leader_user_id']) {
					if($v['role_id'] == $sp_role && $placeholder === 'before') {
						break;
					}
					$user_id[] = $v['leader_user_id'];
					
					break;
				} else if ($placeholder === 'after' && $v['role_id'] < $sp_role && $leaderLineInfo[$key-1]['role_id'] == $sp_role && !in_array($v['leader_user_id'], $progressUid)) {
					$user_id[] = $v['leader_user_id'];
				}
			}else {
				if($v['role_id'] == $sp_role && !in_array($v['leader_user_id'], $progressUid) && $params['operator'] != $v['leader_user_id']) {
