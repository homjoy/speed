...
$app->push(new \Frame\Speed\Middleware\WorkflowApiAuthCheck(array()), '\Frame\Speed\Middleware\ExceptionHandler');
//注册module的namespace
$app->module_namespace = '\\WorkFlowApi\\Modules\\';
...
$app->config->db = function() {
    return \WorkFlowApi\Config\Web\MySQL::config();
};
$app->config->redis = function() {
    return \WorkFlowApi\Config\Web\Redis::config();
};
$app->config->memcache = function() {
    return \WorkFlowApi\Config\Web\Memcache::config();
};
$app->config->remote = function() {
    return \WorkFlowApi\Config\Web\Remote::config();
};
