fml.define('fms/reimburse/modules/BusinessTrip', ['jquery', 'plugin/artTemplate', "component/notify", 'fms/reimburse/modules/Base'], function (require, exports) {
    ...
    Module.prototype = {
        ...
        bindEvent: function () {
            var that = this;
            var applyPersonId = that.templateData.applyPersonid,
                handlePersonId = that.templateData.handlePersonid,
                ctripNo = that.templateData.ctripRequestNo;

            if(applyPersonId && applyPersonId != handlePersonId)
               BaseModule.bindUserCTrip(applyPersonId,ctripNo);
            ...
            $.post('/aj/reimburse/ctrip_detail', {ctripNo: tripNo}, function (resp) {
                ...
                $.each(resp.data.data, function (index, traffic) {
                    ...
                    if (row.length) {
                        ...
                        //读取出的行日期进行绑定-hqAdded
                        var curCtripDateInput = $form.find('.transport-fee .detail-control').eq(index).find('[name=billDate]');
                        console.log('curCtripDateInput',curCtripDateInput);
                        BaseModule.bindDatePicker(curCtripDateInput,traffic.ctripDateStr);
                    }
                })
