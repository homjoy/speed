 ...
 * @returns {*}
 */
function stripBlankNotInPre(str) {
    var startPos = str.search(/<pre.*?>/);
    if (startPos < 0) {
        return str.replace(/[  ]+/g, ' ');
    }
    var endPos = str.search(/<\/pre.*?>/) + 5;
    var strippedHtml = str.substring(0, startPos).replace(/[  ]+/g, ' ');
    //不对 Pre 标签内的东西做处理.
    strippedHtml += str.substring(startPos, endPos);
    //递归调用替换.
    strippedHtml += arguments.callee(str.substring(endPos, str.length));
    return strippedHtml;
}
...
function buffer2String(buffer, start, end) {
    //先用占位符替换换行
    var placeholder = '4GWRfPh4u2Ga7nFXK8xDFk7i9cFrFS0d';
  ...
  }).replace(/<pre[^>]*>[\s\S\r\n]*?<\/pre>/mg, function(preStr) {
      //将pre 标签内的换行先用占位符替换回去
      return preStr.split("\n").join(placeholder);
  }).replace(/\\/g, '\\\\').replace(/[\n\r]+/g, htmlend).replace(/'/g, "\\'")
      .replace(/<pre[^>]*>[\s\S\r\n]*?<\/pre>/mg, function(preStr) {
      //将占位符恢复为换行.
      return preStr.split(placeholder).join('\\n');
  });
