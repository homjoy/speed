    ...
    var table = new SearchTable({
        ...
        columnFormatter:function(column, value){
            ...
            if(this.summaryTable){
                var moneyFieldKeys = ['contractamount','contractmonthamount','price','contract_amount','contract_month_amount'];
                var moneyFlag = false;

                column = column.toLowerCase();

                $.each(moneyFieldKeys,function(index,item){
                    if(column.indexOf(item) != -1) moneyFlag = true;
                });

               if(moneyFlag) return utils.formatCurrency(value);
            }
