fml.define('fms/common/utils', ['jquery', 'component/upload', 'component/notify'], function (require, exports) {
    ...
    exports.formTemplate = {
        ...
        '/contract/dealTask/checkapprove': '<form><textarea placeholder="不超过200字" class="form-control approval-info" style="min-height:80px"></textarea></form>',
        ...
        + '<div class="checkbox"><label><input type="checkbox" id="isChecked" name="is_check" value="1"/>需要进一步审批</label></form>',
     ...
     */
    exports.getCurrencyValue = function getCurrencyValue(text) {
    ...
    exports.multiUpload = function (element, options) {
        ...
        var zoneOptions = {
            ...
            //acceptedFiles: '.pdf,.doc,.docx,.xlsx,.xls,.ppt,.pptx,.zip,.rar'
        };
     ...
     * @param options
     */
    exports.bindUpload = function (btn, options) {
        /**
         * 解析已上传的附件.
         */
        var existFiles = [];
        try {
            var files = $.parseJSON($(btn).attr('data-exists'));
        ...
        $(btn).QueenUpload({
            ...
            onSuccess: function (response, file) {
                ...
                if (options.uploadInput) {
                    $(options.uploadInput).val(filePaths.join(','));
                }
            },
            ...
            onFileRemove: function (file) {
                ...
                if (file.fileId) {
                    ...
                    if (options.deleteInput) {
                        $(options.deleteInput).val(delPath.join(','));
                    }
                }
            }
        });
