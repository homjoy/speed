fml.define('speed/time/modal', ['jquery', 'component/notify', 'plugin/bootbox', 'plugin/store', 'plugin/artTemplate', 'plugin/tokeninput', 'plugin/bootstrap/datetimepicker', 'plugin/moment'], function (require, exports) {
    "use strict";
    var $ = require('jquery');
    ...
    $(document).on('change', ".duration select", function (e) {
        ...
        if (check) {
            if (duration >= 120) {
                notify.warning("预订会议室已达2小时，如发现不合理使用可能会有处罚哦！");
            } else if (duration < 120 && duration > 60) {
                notify.warning("预订会议室超过1小时啦，确定效率不能再提高了吗？");
            }
        }
    });
    ...
    function showModal(title, form, success, remove) {
        var btn_success_name = title == "详情" ? "编辑" : "确定";
        var btn_cancel_name = title == "详情" ? "了解" : "取消";
        var options = {
        ...
        var passModalWizard = store.get('passModalWizard');
        var wizard, count = 10;
        ...
        if (modalForm.length && modalForm.hasClass('both') && !passModalWizard) {
            ...
            wizard.on('click', function () {
                var self = $(this);
                self.remove();
                store.set('passModalWizard', 1, {expires: 365});
            });
            setTimeout(reLoc, 300);
    ...
    return {
            ...
            if (rooms && rooms.length) {
                for (var i = 0, len = rooms.length; i < len; i++) {
                    if (typeof rooms[i].services == 'string') {
                        rooms[i].services = !rooms[i].services ? [] : rooms[i].services.split(',');
                    } else {
        ...
        update: function (type, id, callback, rooms) {
            ...
            if (type == 'meeting') {
                $.getJSON('/aj/meeting/get_book', {book_id: id}, function (resp) {
                    if (resp.code != 200) {
        ...
        cancel: function (type, id, callback) {
            ...
            var url = (type == 'meeting' ? '/aj/meeting/delete' : '/aj/time/delete');
         ...
         * @returns {*}
         */
        getDetail: function (type, id, callback, rooms) {
            if (!callback || typeof callback !== 'function') {
                callback = function () {
                };
            }

            $.getJSON('/aj/time_meeting/get_detail', {type:type,id:id}, function (resp) {
                if (resp.code != 200) {
                    return notify.error('出错了');
                }
                var book = resp.data;
                ...
                var form = artTemplate('time-form-detail', book);
                showModal('详情', form, function () {
                    callback();
                });

                var $btn = $('.time-modal').find('button[data-bb-handler="success"]');
