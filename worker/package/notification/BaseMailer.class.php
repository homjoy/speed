...
class BaseMailer {
    ...
    protected function getCommand() {
        ...
        $header = "FROM:=?UTF-8?B?".base64_encode($this->_from_name)."?=<" . $this->_from . ">\r\n";
        ...
        //主题
        $header .= "Subject:=?UTF-8?B?" . base64_encode($this->_subject) ."?=\r\n";
        //html或者纯文本的邮件声明成这个
    ...
    private function socket() {
       ...
       //设置阻塞模式
        if (socket_set_block($this->_socket) === false) {     // $socket is now blocking
            $this->_errorMessage = socket_strerror(socket_last_error());
            return false;
        }
