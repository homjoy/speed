...
class TemplateEngine
{
    ...
    public static $templateMap = array(
        10  => 'mail/festival_remind.tpl',//无模板
        20 => 'mail/log_notification.tpl',
        30 => 'mail/fms_log_notification.tpl', //财务模板
        40 => 'mail/user_birthday_remind.tpl',
        41 => 'mail/user_entry_anniversary_remind.tpl',
        ...
        125 => 'common/process_receive.tpl',
        126 => 'common/vpn_verify_code.tpl',
        ...
        224 => 'mail/vpn_password_out_warn.tpl',
        225 => 'mail/process_receive.tpl',
        226 => 'mail/mls_volume_remind.tpl',
        227 => 'mail/mfacode_remind.tpl',


        300 => 'mail/old_speed_common.tpl', //老speed 系统使用的公用模板
        ...
        508 => 'mail/meeting_fms_reminder_section.tpl',
        509 => 'mail/meeting_fms_common.tpl', //通用的模板,传递 approveName  content (发送内容)  viewUrl

        // 开放平台
        601 => 'mail/open_app_approval_sender.tpl',
        //节日祝福类模板
        701 => 'common/sms_holiday_wishes.tpl',
    );
...
//        $template = Template::model()->getById($templateId);
    ...
    private static function evaluate($code, array $variables = NULL, $templateId = 0)
    {
        ...
        }else if($templateId > 100 && $templateId < 500){
			include(APP_PATH . '/template/layout.html');
		}else if($templateId > 500 && $templateId < 600){
			//fms
			include(APP_PATH . '/template/layout_fms.html');
		}elseif($templateId > 600){
            //open
            include(APP_PATH . '/template/layout_open.html');
        }
        eval('?>' . $code);
        return ob_get_clean();
        /*return eval('?>' . $code);*/
